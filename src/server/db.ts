...
if (!sqlite.prepare(`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'example_table'`).get()) {
...
const db = new (class Database {
  ...
  private _insert = sqlite.prepare('INSERT INTO example_table (name, value) VALUES (?, ?)');
  ...
  private _update = sqlite.prepare('UPDATE example_table SET value = ? WHERE id = ?');
